...
Factor graphs are a way to unify probabilistic graphical models such as Bayesian networks and Markov networks.  \{ To-do:  I will explain it in more details \}

\underconst

Factor graph algorithm:\\
In the following we use the notational convention of omitting $P(\cdot)$ for probabilities, eg $P(X)$ is written as $(X)$.
...
(G|K) &=& \frac{(K|G)(G)}{(K)} \nonumber\\
      &=& \frac{(G)}{(K)} \sum_H (K|H)(H|G) \nonumber\\
      &=& (G) \sum_H \frac{(H|K)}{(H)} \sum_{B_i} (H|G,B_1,B_2,...)(B_i|G) \nonumber\\
      &=& (G) \sum_H \frac{(H|K)}{(H)} \sum_{B_i} (H|G,B_1,B_2,...)(B_1)(B_2)...
\end{eqnarray*}
...
\subsection{The ``Prolog trick''}

Normally, a probabilistic inference engine needs to generate a propositional Bayesian network in order to evaluate the answer for a query.  What I call the Prolog trick is a way to calculate probabilities approximately without involving the complexity of Bayesian network belief propagation.  It works like SLD resolution in Prolog.

We will illustrate the trick with 2 examples:

\textbf{Single factor case.}  Given $A \leftarrow B$.

We're seeking the value of $P(A)$ as the truth value of node ``A''.
