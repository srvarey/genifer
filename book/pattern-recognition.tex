...
\end{equation}
with $x : \beta$ and $f, g : \beta \rightarrow \alpha $.  This definition is given in Church's type theory in 1940.  It defines equality for any function / predicate / proposition.
...
\begin{eqnarray}
\forall f [f x = f y] &\Rightarrow& x = y \\
\# f [f x = f y]      &\Rightarrow& x \approx y.
\label{eqn:inverted-extensionality}
\end{eqnarray}
Notice that if we use higher-order logic, $f$ can be unified with n-ary predicates with other arguments, eg
 $$f(x) = \mbox{lives-in}(x, \mbox{water}).$$
So this is a very powerful way of expressing all forms of similarities.

\{ To-do:  I need to develop the above theory further.  Milner's Context Lemma in domain theory seems to have some semblance to what we're doing here.  \}
